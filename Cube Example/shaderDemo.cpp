...
void renderScene(void) {
	...
	glDrawElements(GL_TRIANGLES, gd.nTris * 3, GL_UNSIGNED_INT, 0);
...
void importOffModel()
{
	...
	gd.nTris = 0;
	int count = 0;
	gd.pVertices = new float[ld.verts.size() * 4];
	for (int i = 0; i<ld.verts.size(); i++) {
		std::cout << "vert " << i << " " ;
		gd.pVertices[count] = (ld.verts[i])[0]; count++;
		std::cout << gd.pVertices[count-1] << " ";
		gd.pVertices[count] = (ld.verts[i])[1]; count++;
		std::cout << gd.pVertices[count - 1] << " ";
		gd.pVertices[count] = (ld.verts[i])[2]; count++;
		std::cout << gd.pVertices[count - 1] << " ";
		gd.pVertices[count] = 1; count++;
		std::cout << gd.pVertices[count - 1] << endl;
		gd.nVertices++;
	}
	std::cout << "no vert" << gd.nVertices << endl;
	count = 0;
	gd.pIndices = new unsigned int[ld.tris.size() * 3];
	for (int i = 0; i<ld.tris.size(); i++) {
		std::cout << "tri " << i << " ";
		gd.pIndices[count] = (ld.tris[i])[0]; count++;
		std::cout << gd.pIndices[count - 1] << " ";
		gd.pIndices[count] = (ld.tris[i])[1]; count++;
		std::cout << gd.pIndices[count - 1] << " ";
		gd.pIndices[count] = (ld.tris[i])[2]; count++;
		std::cout << gd.pIndices[count - 1] << endl;
		gd.nTris++;
	}
	std::cout << "no tris" << gd.nTris << endl;
	count = 0;
	gd.pNormals = new float[ld.tris.size() * 3];
	for (int i = 0; i<ld.tris.size(); i++) {
		std::cout << "normal " << i << " ";
		gd.pNormals[count] = (ld.trinormal[i])[0]; count++;
		std::cout << gd.pNormals[count - 1] << " ";
		gd.pNormals[count] = (ld.trinormal[i])[1]; count++;
		std::cout << gd.pNormals[count - 1] << " ";
		gd.pNormals[count] = (ld.trinormal[i])[2]; count++;
		std::cout << gd.pNormals[count - 1] << endl;
		
	}
	std::cout << "no normal" << gd.nTris << endl;
}
...
void initOpenGL()
{
	...
	glBufferData(
		GL_ARRAY_BUFFER, 		// target to which the buffer object is bound
		sizeof(float)*gd.nVertices * 4, 		// size in bytes of the buffer object's new data store
		gd.pVertices, 				// pointer to data, or NULL if no data is to be copied
//		sizeof(vertices), 		// size in bytes of the buffer object's new data store
//		vertices, 				// pointer to data, or NULL if no data is to be copied
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * gd.nTris *3, gd.pIndices, GL_STATIC_DRAW);
